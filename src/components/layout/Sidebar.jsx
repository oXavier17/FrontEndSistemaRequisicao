...
import { NavLink, useNavigate, useLocation } from 'react-router-dom'; // ← adicione useLocation
...
export default function Sidebar() {
  ...
  useEffect(() => {
    // Recarrega sempre que a rota muda
    api.get('/requisicoes')
      .then(r => {
  ...
  }, [location.pathname]); // ← recarrega a cada mudança de página
  ...
  const navItems = [
    ...
    { to: '/departamentos', icon: Building2,       label: 'Departamentos', show: isAdmin },
          ...
          <div className="user-info">
            ...
            <div className="user-role">
              {usuario?.tipoPerfil === 1 ? 'Administrador' :
               usuario?.tipoPerfil === 2 ? 'Funcionário' : 'Requisitante'}
            </div>
          </div>
          <div className="logout-btn" title="Sair" onClick={handleLogout}>
            <LogOut size={15} />
          </div>
        </div>
      </div>
